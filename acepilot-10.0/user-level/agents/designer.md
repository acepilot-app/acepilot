...
**Asset integrity check** (v8.1) — for every diff that touches HTML/templates:

- Verify all `<img src="...">` paths exist (glob for the file). Missing image = 🔴.
- Verify all CSS `url(...)` paths exist. Missing background/font = 🔴.
- Verify all `<link href="...">` and `<script src="...">` paths exist. Missing = 🔴.
- Check for hardcoded `http://` on HTTPS sites. Mixed content = 🔴.
- Check for relative paths that break when deployed (e.g., `images/logo.png` vs `/images/logo.png`).
- Check for references to localhost, 127.0.0.1, or staging URLs in production code = 🔴.
