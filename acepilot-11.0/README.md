...
A self-evolving autonomous engineer for Claude Code with a built-in growth engine. Not a fork, not a wrapper, not a separate tool. It's 12 config files (+5 templates) that install into Claude Code's native config structure, adding a `/acepilot` command that transforms how Claude Code behaves for the rest of that session.

Without it, Claude Code runs 100% stock. With it activated, Claude Code absorbs your full project context, captures proven workflows as replayable playbooks, auto-tunes its own execution engine from data, chains objectives across sessions, deploys to any platform, and auto-generates user acquisition infrastructure (SEO, comparison pages, viral loops, launch assets). Ship today. Ship 42% faster by session 10. Grow users from day one.
