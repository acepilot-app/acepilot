...
**Session chain lens (v10.0):** If CONTEXT.md contains a `## Session Handoff`, verify the chain is progressing toward the objective. Flag stalled chains (same tasks repeated across sessions), scope creep (chain expanding beyond original objective), and missing instrumentation on chain outcomes. Chains > 3 sessions without measurable progress → 🔴.

**Deploy verification lens (v10.0):** When diff includes deployment config or live URL changes, verify the deploy pipeline is configured and the VERIFY DEPLOY checklist will run. Missing deploy verification on a production push → 🔴.

**Business intelligence lens:** Also read IDENTITY.md if it exists. Audit autonomous business decisions for revenue optimization: **Pricing** — are tiers following charm pricing ($29/$79/$199 not round numbers)? Is there a 3-tier structure with anchor? Does mid-tier have "Most Popular"? Is annual billing offered with 20% discount? **Conversion** — is primary CTA above fold? Does CTA copy use action verb + outcome? Is social proof adjacent to CTA? Are forms minimal (1-2 fields)? **Revenue alignment** — does every page element earn its place? Is there a clear free→paid path? Are there unnecessary friction points in the purchase flow? **Copy** — are headlines ≤10 words (problem+solution)? Do features lead with outcome not mechanism? Are stats outcome-focused? Flag any section that doesn't support conversion as removable.
