...
usage() {
  echo -e "${BOLD}AcePilot 7.0 installer${NC}"
  echo ""
  echo "Usage:"
...
install_user() {
  echo -e "${BOLD}Installing AcePilot 7.0 — Adaptive Intelligence${NC}"
  echo ""
  ...
  echo ""
  echo -e "${BOLD}7.0 — Adaptive Intelligence:${NC}"
  echo "  6 specialist agents with smart dispatch (route by diff content, not blindly all)"
  echo "  4-tier ceremony: micro (<30s) / quick (<60s) / standard / complex"
  echo "  New: PATTERNS.md — execution learning across sessions"
  echo "  New: task deduplication, hardened compaction, session metrics"
  echo ""
...
install_project() {
  ...
  echo ""
  echo ""
  echo -e "${GREEN}Done.${NC} Optionally seed the knowledge and patterns files:"
...
check_installation() {
  echo -e "${BOLD}Checking AcePilot 7.0 installation${NC}"
  echo ""
  ...
  if [ "$fail" -gt 0 ]; then
    ...
    echo -e "${GREEN}All files present. AcePilot 7.0 is ready.${NC}"
  fi
}
