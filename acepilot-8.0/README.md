...
A bolt-on autonomous execution system for Claude Code. Not a fork, not a wrapper, not a separate tool. It's 12 config files (+3 templates) that install into Claude Code's native config structure, adding a `/acepilot` command that transforms how Claude Code behaves for the rest of that session.
...
cd acepilot
./acepilot-8.0/install.sh              # Install user-level files
cd your-project && "/path/to/acepilot-8.0/install.sh" --project  # Add hooks to a project
```
...
> **iCloud Drive / OneDrive / Dropbox:** If AcePilot is stored in a cloud-synced folder whose path contains spaces (e.g. `~/Library/Mobile Documents/...`), quote the path when invoking the installer: `"/path/with spaces/acepilot-8.0/install.sh"`. The installer itself is path-safe — all internal file operations use proper quoting.
...
./acepilot-8.0/install.sh

# Install project-level files (do this in each project you want AcePilot on)
cd your-project
"/path/to/acepilot-8.0/install.sh" --project
```

The installer detects existing files: missing files are created, identical files are skipped, changed files are backed up (with timestamp) before overwriting. Run `"./acepilot-8.0/install.sh" --check` to verify installation, or `--uninstall` to remove.
...
cp "acepilot-8.0/user-level/CLAUDE.md" ~/.claude/CLAUDE.md
cp "acepilot-8.0/user-level/settings.json" ~/.claude/settings.json
mkdir -p ~/.claude/commands ~/.claude/agents
cp "acepilot-8.0/user-level/commands/acepilot.md" ~/.claude/commands/acepilot.md
cp "acepilot-8.0/user-level/agents/researcher.md" ~/.claude/agents/researcher.md
cp "acepilot-8.0/user-level/agents/reviewer.md" ~/.claude/agents/reviewer.md
cp "acepilot-8.0/user-level/agents/designer.md" ~/.claude/agents/designer.md
cp "acepilot-8.0/user-level/agents/security.md" ~/.claude/agents/security.md
cp "acepilot-8.0/user-level/agents/architect.md" ~/.claude/agents/architect.md
cp "acepilot-8.0/user-level/agents/strategist.md" ~/.claude/agents/strategist.md

# Copy project-level files (do this in each project you want AcePilot on)
cd your-project
mkdir -p .claude/rules .claude/state
cp "/path/to/acepilot-8.0/project-level/settings.json" .claude/settings.json
cp "/path/to/acepilot-8.0/project-level/rules/browser.md" .claude/rules/browser.md

# Optionally, seed the knowledge file (auto-seeds on first run, but you can pre-populate)
cp "/path/to/acepilot-8.0/KNOWLEDGE-TEMPLATE.md" .claude/state/KNOWLEDGE.md
cp "/path/to/acepilot-8.0/PATTERNS-TEMPLATE.md" .claude/state/PATTERNS.md
```
...
| 7.0     | Adaptive Intelligence. Smart specialist dispatch (route by diff content, not blindly all 5 — even in god mode). 4-tier ceremony: micro (<30s), quick (<60s), standard, complex. PATTERNS.md state file for execution learning across sessions (specialist ROI, failure recovery, false positives). Task deduplication via TASKS.md + git log cross-reference. Hardened compaction with post-verification checklist. Session metrics (specialist calls, PASS rate, findings). Knowledge freshness markers (>30d entries flagged). `god --full-team` preserves v6.4 behavior. Directory: `acepilot-8.0/`. |
