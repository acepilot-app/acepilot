...
usage() {
  echo -e "${BOLD}AcePilot 8.1 installer${NC}"
  echo ""
  echo "Usage:"
...
install_user() {
  echo -e "${BOLD}Installing AcePilot 8.1 — Metrics-Aware Intelligence${NC}"
  echo ""
  ...
  echo ""
  echo -e "${BOLD}8.1 — Metrics-Aware Intelligence:${NC}"
  echo "  METRICS.md: paste analytics numbers, AcePilot prioritizes by real impact"
  echo "  Deploy verification: never reports done with broken images or missing assets"
  echo "  Asset integrity: @designer catches broken paths before they go live"
  echo "  Gates weight by traffic: high-traffic pages get stricter review"
  echo ""
...
check_installation() {
  echo -e "${BOLD}Checking AcePilot 8.1 installation${NC}"
  echo ""
  ...
  if [ "$fail" -gt 0 ]; then
    ...
    echo -e "${GREEN}All files present. AcePilot 8.1 is ready.${NC}"
  fi
}
