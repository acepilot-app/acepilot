...
usage() {
  echo -e "${BOLD}AcePilot 9.0 installer${NC}"
  echo ""
  echo "Usage:"
...
install_user() {
  echo -e "${BOLD}Installing AcePilot 9.0 — Autonomous Business Intelligence${NC}"
  echo ""
  ...
  echo ""
  echo -e "${BOLD}9.0 — Autonomous Business Intelligence:${NC}"
  echo "  Say 'website' — AcePilot decides tech, design, copy, pricing, and ships it"
  echo "  Directive Expansion Engine: minimal input → full business specification"
  echo "  IDENTITY.md: project identity persists across sessions"
  echo "  Revenue-first decisions: every choice optimizes for revenue generation"
  echo "  Business Decision Framework: pricing psychology, conversion, analytics defaults"
  echo ""
...
check_installation() {
  echo -e "${BOLD}Checking AcePilot 9.0 installation${NC}"
  echo ""
  ...
  if [ "$fail" -gt 0 ]; then
    ...
    echo -e "${GREEN}All files present. AcePilot 9.0 is ready.${NC}"
  fi
}
